...
class widget_NameElement extends widget_AbstractElement {
	...
	function renderElement() {
		...
		//	if($elementValue == "" || is_null($elementValue))
			...
			$out =	sfl("<select name='%s[title]' id='form_%s' class='name_title' >%s</select>", 
							$this->getName(),
							$this->getName(),
							$titleOptions
	
						);
			$out .=	sfl("<input type='text' name='%s[firstName]' value='%s' id='form_%s' class='name_firstName' class='text' />", 
							$this->getName(),
							parent::makeInputSafe($name['firstName']),
							$this->getName()
	
						);
			$out .=	sfl("<input type='text' name='%s[lastName]' value='%s' id='form_%s' class='name_lastName' class='text' />", 
							$this->getName(),
							parent::makeInputSafe($name['lastName']),
							$this->getName()
