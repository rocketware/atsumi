	...
	function renderElement() {
		...
		foreach($this->options as $value => $option) {
			if(strval($elementValue) == strval($value)) {
				$usedSuggested = true;
				$html .= sfl('<div class="inputSuggestionOption"><input type="radio" name="%s" value="%s" checked="checked"  /> %s</div>',
									$this->getName(),
									parent::makeInputSafe($value),
									$option
				);
			} else
				$html .= sfl('<div class="inputSuggestionOption"><input type="radio" name="%s" value="%s" /> %s</div>',
									$this->getName(),
									parent::makeInputSafe($value),
									$option
				);
